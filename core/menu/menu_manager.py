...
class MenuSystem:
    ...
    def show_networks(self, networks):
        ...
        # 构建WiFi列表文本
        wifi_text = "扫描到的WiFi网络:\n\n"
        for i, network in enumerate(networks, 1):
            # 限制WiFi名称长度
            network_name = network if len(network) <= 15 else network[:15] + "..."
            wifi_text += f"{i}. {network_name}\n"
        
        wifi_text += "\n使用摇杆上下滚动查看\n按BTN1退回菜单"
        
        # 使用DisplayManager的wait_for_button_with_text功能
        try:
            print("显示WiFi列表，支持滚动浏览")
            # 使用wait_for_button_with_text支持滚动显示
