...
class DeriveChatUtils:
    ...
    def get_text_prompt(self, prompt_type):
        ...
        prompts = {
            ...
            'mood_extraction': (
                """你是一个心情分析专家。用户通过语音表达了他们当前的心情状态，但语音识别可能不够完美。
                
                请你帮助：
                1. 理解用户想要表达的核心心情和感受
                2. 优化表达方式，使其更适合生成史莱姆性格
                3. 如果识别文本不清楚，请根据能理解的部分推测合理的心情状态
                4. 保持用户的原始情感色彩，不要过度修饰
                ...
                示例：
                输入："今天...心情...很放松...想要...安静地方"
                输出："今天感觉很放松，内心很平静，想要找一个安静美好的地方，静静地享受这种宁静的氛围。"
                """,
                "语音识别文本：\"{raw_voice_text}\"\n\n请提取并优化用户的心情描述，使其适合生成个性化的史莱姆角色。"
            ),
            'slime_description': (
                "你是一个专业的角色与环境描述师。请根据这个史莱姆的性格特点，生成一个完整的描述，包括：1)史莱姆的外观细节（颜色、质地、表情、特殊特征）；2)史莱姆所处的环境氛围（与其性格相匹配的环境细节）；3)体现性格的视觉元素和行为表现。描述要精确具体且富有画面感，适合用于AI图像生成，控制在150-200字内。不要使用过于抽象的描述，要有具体的视觉元素。",
                "根据这个性格描述一下史莱姆的外观和所处环境：{text}"
            ),
            'greeting': (
